...
`POST /v1/frames/select` consumes staged media under the strict `source-frame.v1` contract. The request pins source SHA-256 and byte count, selects either the first actual source frame at or after a relative timestamp or an exact zero-based source frame index, and optionally requests a JPEG. Its response returns the authoritative source frame index and relative timestamp, the source checksum, and nullable base64 image bytes. Timestamp extraction shares the preparation manifest's FFprobe PTS rounding and source-start normalization; it never estimates an index from average FPS. The JPEG is the full source frame, decoded with an exact frame-index filter, and is bounded to 8 MiB. The caller applies the fixed Track-view crop. The endpoint shares processing admission, media limits, and an end-to-end deadline, removes claimed scratch on completion, and persists no image or artifact. Rejections use bounded versioned JSON and non-200 status codes.

The canonical `tests/fixtures/source-frame/contracts.json` fixtures cover both request selections, accepted responses, safe error envelopes, and rejected payloads. Python Pydantic and Worker Zod tests consume the same file; Python also checks invalid requests through the HTTP endpoint.
