...
if [[ "${RC_MECH_REQUIRE_REMOTE_CONFIG:-0}" == "1" ]]; then
	...
	if ! pnpm exec wrangler secret list --format json | jq -e 'map(.name) | (["APP_URL", "BETTER_AUTH_SECRET", "OWNER_EMAIL", "EMAIL_FROM", "R2_ACCOUNT_ID", "R2_ACCESS_KEY_ID", "R2_SECRET_ACCESS_KEY", "GPU_ACCESS_CLIENT_ID", "GPU_ACCESS_CLIENT_SECRET", "INFERENCE_PROFILE_JSON"] - .) == []' >/dev/null; then
