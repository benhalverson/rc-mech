...
export const isSupportedPhotoType = (contentType: string): boolean =>
	(PHOTO_CONTENT_TYPES as readonly string[]).includes(
		contentType.toLowerCase(),
	);

export const validatePhotoMetadata = (
	value: PhotoMetadata,
): string | undefined => {
	...
	if (!value.fileName.trim() || value.fileName.length > 255)
		return 'Invalid photo file name';
	return undefined;
};

export const photoObjectKey = (carId: string, photoId: string): string =>
	`cars/${carId}/photos/${photoId}`;

export const normalizePhotoOrder = <
	T extends { sortOrder: number; createdAt: string; id: string },
>(
	photos: T[],
): T[] =>
	[...photos].sort(
...
export const isCompletePhotoOrder = <T extends { id: string }>(
	photos: T[],
	orderedIds: string[],
): boolean => {
...
export const primaryAfterDelete = <
	T extends { id: string; sortOrder: number; createdAt: string },
>(
	photos: T[],
	deletedId: string,
): string | undefined =>
