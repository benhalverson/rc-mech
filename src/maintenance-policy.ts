...
export const addCalendarInterval = (
	baselineAt: string,
	...
	const date = new Date(
		Date.UTC(
			...
			month - 1,
			unit === 'months'
				? Math.min(start.day, new Date(Date.UTC(year, month, 0)).getUTCDate())
				: 1,
		),
	);
...
export const calculateMaintenanceDue = (
	...
	const sessionsSinceBaseline = Math.max(
		0,
		input.currentSessionCount - input.baselineSessionCount,
	);
	...
	const calendarDue =
		dateDueAt !== null &&
		new Date(input.now).getTime() >= new Date(dateDueAt).getTime();
	const sessionDueAtCount =
		input.intervalSessions === null
			? null
			: input.baselineSessionCount + input.intervalSessions;
	const sessionsDue =
		sessionDueAtCount !== null &&
		input.currentSessionCount >= sessionDueAtCount;
	...
	const overdue =
		dateDueAt !== null &&
		calendarDue &&
		localDate(localParts(input.now, input.timezone)) >
			localDate(localParts(dateDueAt, input.timezone));
	return {
