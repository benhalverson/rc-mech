...
import { canDeleteDriveSession, canEditDriveSession, isIanaTimezone, presentDateTime } from "./drive-session-policy.ts";
...
test("session lifecycle makes soft-deleted sessions immutable", () => {
	...
	assert.equal(canEditDriveSession({ deletedAt: "2026-08-03T00:00:00.000Z" }), false);
	assert.equal(canDeleteDriveSession({ deletedAt: "2026-08-03T00:00:00.000Z" }), false);
});

test("date presentation is stable for the configured timezone", () => {
	assert.deepEqual(presentDateTime("2026-08-03T07:30:05.000Z", "America/Los_Angeles"), {
