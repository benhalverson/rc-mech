...
const base = (overrides: Partial<Parameters<typeof calculateMaintenanceDue>[0]> = {}) => ({
	...
	intervalUnit: "months" as const, intervalValue: 1, intervalSessions: null, currentSessionCount: 10,
	now: "2026-02-28T12:00:00.000Z", timezone: "UTC", ...overrides,
});

test("calendar intervals support days, weeks, and clamped months", () => {
	assert.equal(addCalendarInterval("2026-01-01T12:00:00.000Z", "days", 10, "UTC"), "2026-01-11T00:00:00.000Z");
	assert.equal(addCalendarInterval("2026-01-01T12:00:00.000Z", "weeks", 2, "UTC"), "2026-01-15T00:00:00.000Z");
	assert.equal(addCalendarInterval("2026-01-31T12:00:00.000Z", "months", 1, "UTC"), "2026-02-28T00:00:00.000Z");
});
...
test("date threshold is due and becomes overdue after its local date", () => {
	...
	assert.equal(calculateMaintenanceDue(base({ now: "2026-03-01T12:00:00.000Z" })).dueStatus, "overdue");
});
...
test("calendar calculations use the owner timezone", () => {
	const result = calculateMaintenanceDue(base({ baselineAt: "2026-03-08T07:30:00.000Z", intervalUnit: "days", intervalValue: 1, now: "2026-03-08T06:59:00.000Z", timezone: "America/Los_Angeles" }));
