...
export const gpuWaiters = sqliteTable('gpu_waiters', {
	segmentId: text('segment_id').primaryKey(),
	deadlineAt: integer('deadline_at').notNull(),
	kind: text('kind').notNull(),
	ordinal: integer('ordinal').notNull(),
}, (table) => [uniqueIndex('gpu_waiters_ordinal').on(table.ordinal), index('gpu_waiters_fifo').on(table.ordinal)]);

export const gpuLease = sqliteTable('gpu_lease', {
