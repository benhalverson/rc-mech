...
RC Mech uses a one-way Angular feature architecture: components own presentation and local interaction, route-provided NgRx Signal Stores own workflow state and reactive command outcomes, focused gateways and capability services own external systems, and pure functions own transformations. This keeps component dependencies visible without moving the same complexity into catch-all services or umbrella stores.
