...
describe('ConsumableMaintenance', () => {
	...
	describe('component', () => {
		...
		it('disables history creation when every car is archived', () => {
			...
			fixture.detectChanges();
			expect(
				fixture.nativeElement.querySelector('.consumable-form'),
			).toBeNull();
		});
		...
		it('covers form validation focus paths, tire detail validation, and action guard', () => {
			...
			expectFieldError(
				'#entry-rear-details-error',
				'rear tire details or cost',
			);
			...
			const workflow = TestBed.inject(ConsumableStore);
			workflow.mutate({
				kind: 'save',
				mode: 'create',
				carId: 'car-1',
			...
			http.expectNone(
				(request) =>
					request.method === 'POST' &&
					request.url === '/api/v1/cars/car-1/consumable-maintenance' &&
		...
		it('rejects invalid optional costs even when field validation is bypassed', () => {
			const app = fixture.componentInstance as unknown as Harness;
			app.openCreate();
			Object.defineProperty(app.entryFields(), 'invalid', {
				value: () => false,
			});
			app.form.set({
				...app.form(),
		...
		it('filters history and labels all entry variants', () => {
			...
			expect(app.areaLabel({ ...archived, axle: undefined })).toBe(
				'front axle',
			);
