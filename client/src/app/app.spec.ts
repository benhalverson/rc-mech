...
type TestAction = (...args: unknown[]) => unknown;
type AppTestHarness = {
	email: TestSignal<string>;
	passkeys: TestSignal<unknown[]>;
	...
	sessionForm: TestSignal<Record<string, string>>;
	timezoneForm: TestSignal<string>;
	openEditComponent: TestAction;
	openReplaceComponent: TestAction;
	componentEditing: TestSignal<boolean>;
	carForm: TestSignal<Record<string, string>>;
...
describe('App', () => {
	...
	afterEach(() => {
		...
		http
			.match(
				(request) =>
					request.url.endsWith('/service-records') && request.method === 'GET',
			)
			.forEach((request) => {
		...
		http
			.match(
				(request) =>
					request.url.endsWith('/photos') && request.method === 'GET',
			)
			.forEach((request) => {
	...
	const flushDriveSessions = (sessions: unknown[] = []) => {
		http
			.expectOne(
				(request) =>
					request.url === '/api/v1/cars/car-1/drives' &&
	...
	const currentMotor = {
		...
		slot: 'motor',
		slotType: 'standard',
		name: 'Competition motor',
		manufacturer: 'Reedy',
		model: 'M3',
		serialNumber: 'M-002',
		notes: 'Fresh bearings',
	...
	it('requests a magic link with the entered email and a same-origin callback', () => {
		...
		(fixture.componentInstance as unknown as AppTestHarness).email.set(
			'owner@example.test',
		);
		fixture.detectChanges();
		(
		...
		).toContain('If that address is allowed');
	});
	...
	it('signs in with a browser passkey through Better Auth endpoints', async () => {
		const previousPublicKeyCredential = Object.getOwnPropertyDescriptor(
			globalThis,
			'PublicKeyCredential',
		);
	...
	it('lists, renames, and revokes a named passkey', async () => {
		...
		expect(row).toBeNull();
		(fixture.componentInstance as unknown as AppTestHarness).passkeys.set([
			{
				id: 'pk-1',
		...
		(
			fixture.nativeElement.querySelector(
				'.passkey-row .text-button',
			) as HTMLButtonElement
		).click();
		(
			fixture.componentInstance as unknown as AppTestHarness
		).editedPasskeyName.set('Phone');
		fixture.detectChanges();
		(
			fixture.nativeElement.querySelector('.rename-form') as HTMLFormElement
	...
	it('renders the garage and signs out an authenticated owner', () => {
		...
		(
			fixture.nativeElement.querySelector('button.quiet') as HTMLButtonElement
		).click();
	...
	it('loads a car build sheet with current installations and replacement history', () => {
		...
		const request = http.expectOne(
			(req) =>
				req.url === '/api/v1/cars/car-1/components' &&
				req.params.get('history') === 'true',
		);
	...
	it('adds a custom-slot component using the explicit build-sheet contract', () => {
		...
		http
			.expectOne((req) => req.url === '/api/v1/cars/car-1/components')
			.flush({ components: [], history: true });
		flushDriveSessions();
		fixture.detectChanges();

		(fixture.componentInstance as unknown as AppTestHarness).openAddComponent();
		(fixture.componentInstance as unknown as AppTestHarness).componentForm.set({
			slotType: 'custom',
			slot: 'front-sway-bar',
			name: 'Sway bar',
			manufacturer: 'XRAY',
		...
		(fixture.componentInstance as unknown as AppTestHarness).saveComponent();
	...
	it('records, edits, archives, and counts drive sessions in the run log', () => {
		...
		(
			fixture.nativeElement.querySelector(
				'.session-log .section-heading .button',
			) as HTMLButtonElement
		).click();
		...
		(
			fixture.nativeElement.querySelector(
				'.session-row .text-button',
			) as HTMLButtonElement
		).click();
		(fixture.componentInstance as unknown as AppTestHarness).sessionForm.update(
			(form: Record<string, string>) => ({ ...form, notes: 'Updated setup' }),
		);
		fixture.detectChanges();
		(
			fixture.nativeElement.querySelector('.session-form') as HTMLFormElement
		...
		remove.flush({
			driveSession: { ...saved, deletedAt: '2026-08-03T18:00:00.000Z' },
		});
		flushDriveSessions([
			{
				...saved,
				notes: 'Updated setup',
				deletedAt: '2026-08-03T18:00:00.000Z',
			},
		]);
		fixture.detectChanges();
		expect(fixture.nativeElement.textContent).toContain(
	...
	it('saves an IANA timezone and uses it for session display', () => {
		createFixture();
		showSignedIn();
		(fixture.componentInstance as unknown as AppTestHarness).timezoneForm.set(
			'America/New_York',
		);
		fixture.detectChanges();
		(
			fixture.nativeElement.querySelector('.timezone-form') as HTMLFormElement
		).dispatchEvent(new Event('submit'));
	...
	it('keeps archived session history visible but blocks new sessions', () => {
		...
		expect(fixture.nativeElement.textContent).toContain(
			'new sessions cannot be recorded',
		);
		expect(
			fixture.nativeElement.querySelector(
				'.session-log .section-heading .button',
	...
	it('edits current details and replaces through separate lifecycle endpoints', () => {
		...
		http
			.expectOne((req) => req.url === '/api/v1/cars/car-1/components')
			.flush({ components: [currentMotor, previousMotor], history: true });
		flushDriveSessions();

		(fixture.componentInstance as unknown as AppTestHarness).openEditComponent(
			currentMotor,
		);
		(
			fixture.componentInstance as unknown as AppTestHarness
		).componentForm.update((form: Record<string, string>) => ({
			...form,
			model: 'M3 Pro',
		}));
		(fixture.componentInstance as unknown as AppTestHarness).saveComponent();
		...
		http
			...
			.flush({
				components: [{ ...currentMotor, model: 'M3 Pro' }, previousMotor],
				history: true,
			});
		...
		(
			...
			...form,
			name: 'New motor',
			manufacturer: 'Muchmore',
			model: 'Fleta',
		}));
		(fixture.componentInstance as unknown as AppTestHarness).saveComponent();
		const replace = http.expectOne(
			'/api/v1/cars/car-1/components/component-2/replace',
		);
		expect(replace.request.method).toBe('POST');
		expect(replace.request.body.slotType).toBe('standard');
		replace.flush({
			previous: { ...currentMotor, removedAt: '2026-08-03T00:00:00.000Z' },
			component: {
				...currentMotor,
				id: 'component-3',
				name: 'New motor',
		...
		http
			...
			.flush({
				components: [
					{ ...currentMotor, id: 'component-3', name: 'New motor' },
					previousMotor,
					{ ...currentMotor, removedAt: '2026-08-03T00:00:00.000Z' },
				],
				history: true,
			});
		fixture.detectChanges();
	...
	it('keeps archived build sheets visible but blocks component writes', () => {
		...
		http
			.expectOne((req) => req.url === '/api/v1/cars/car-1/components')
			.flush({ components: [currentMotor], history: true });
		flushDriveSessions();
		fixture.detectChanges();

		expect(fixture.nativeElement.textContent).toContain(
			'read-only while the car is archived',
		);
		expect(
			fixture.nativeElement.querySelector('.build-sheet .button'),
		).toBeNull();
		(fixture.componentInstance as unknown as AppTestHarness).openAddComponent(
			'motor',
		);
	...
	it('shows a loading state and recovers from a garage error', () => {
		...
		fixture.detectChanges();
		expect(
			fixture.nativeElement.querySelector('.error-state')?.textContent,
		).toContain('garage could not be loaded');

		(
			fixture.nativeElement.querySelector(
				'.error-state .text-button',
			) as HTMLButtonElement
		).click();
		http.expectOne('/api/v1/cars').flush({ cars: [] });
		fixture.detectChanges();
	...
	it('creates a car with the garage fields and returns to its detail view', () => {
		...
		(
			fixture.nativeElement.querySelector(
				'.garage-heading .button',
			) as HTMLButtonElement
		).click();
		(fixture.componentInstance as unknown as AppTestHarness).carForm.set({
			name: 'Red Runner',
			make: 'Associated',
			model: 'B6.4',
			scale: '1/10',
		...
		fixture.detectChanges();
		(
			fixture.nativeElement.querySelector('.car-form') as HTMLFormElement
		).dispatchEvent(new Event('submit'));
		...
		expect(
			fixture.nativeElement.querySelector('#car-detail-title')?.textContent,
		).toContain('Red Runner');
	...
	it('edits, archives, and restores a car while keeping archived records inspectable', () => {
		...
		(fixture.componentInstance as unknown as AppTestHarness).selectedCarId.set(
			'car-1',
		);
		(fixture.componentInstance as unknown as AppTestHarness).carView.set(
			'detail',
		);
		fixture.detectChanges();
		...
		(fixture.componentInstance as unknown as AppTestHarness).carForm.update(
			(form: Record<string, string>) => ({ ...form, name: 'Red Runner v2' }),
		);
		fixture.detectChanges();
		(
			fixture.nativeElement.querySelector('.car-form') as HTMLFormElement
		...
		http
			.expectOne('/api/v1/cars')
			.flush({ cars: [{ ...workshopCar, name: 'Red Runner v2' }] });
		fixture.detectChanges();

		(
			fixture.nativeElement.querySelector('.danger-button') as HTMLButtonElement
		).click();
		...
		const archivedList = http.expectOne(
			(request) =>
		...
		http
			...
			.flush({ cars: [{ ...archivedCar, archivedAt: null }] });
		fixture.detectChanges();
	...
	it('can inspect archived cars separately from the active garage', () => {
		...
		(
			fixture.nativeElement.querySelector(
				'.garage-toolbar .text-button',
			) as HTMLButtonElement
		).click();
		const request = http.expectOne(
			(req) =>
				req.url === '/api/v1/cars' && req.params.get('archived') === 'all',
		);
		request.flush({
			cars: [{ ...workshopCar, archivedAt: '2026-07-01T00:00:00.000Z' }],
		});
		fixture.detectChanges();

		expect(
			fixture.nativeElement.querySelector('.car-row')?.textContent,
		).toContain('Archived');
	...
	it('loads component history and supports adding and replacing an installation', () => {
		...
		expect(
			fixture.nativeElement.querySelector('.component-current')?.textContent,
		).toContain('Tekin');
		expect(
			fixture.nativeElement.querySelector('.component-history'),
		).toBeTruthy();

		(
			fixture.nativeElement.querySelector(
				'.build-sheet .section-heading .button',
			) as HTMLButtonElement
		).click();
		(fixture.componentInstance as unknown as AppTestHarness).componentForm.set({
			slotType: 'custom',
		...
		fixture.detectChanges();
		(
			fixture.nativeElement.querySelector('.component-form') as HTMLFormElement
		).dispatchEvent(new Event('submit'));
		...
		(fixture.componentInstance as unknown as AppTestHarness).components.set([
			current,
			previous,
		]);
		(
			fixture.componentInstance as unknown as AppTestHarness
		).openReplaceComponent(current);
		(fixture.componentInstance as unknown as AppTestHarness).componentForm.set({
			slotType: 'standard',
			slot: 'motor',
			name: 'Motor v3',
		...
		expect(replace.request.body).toMatchObject({
			slotType: 'standard',
			model: 'RX9',
		});
		replace.flush({
			component: { ...current, id: 'component-4', name: 'Motor v3' },
			previous,
		});
		http
			.expectOne(
			...
			.flush({
				components: [
					previous,
					current,
					{ ...current, id: 'component-4', name: 'Motor v3', removedAt: null },
				],
			});
	});
	...
	it('shows a generic recovery message and removes callback errors from the URL', () => {
		window.history.replaceState(
			{},
			'',
			'/?error=invalid_token&error_description=expired&view=access',
		);
		createFixture();
		showSignedOut();
		...
		expect(
			fixture.nativeElement.querySelector('[role="status"]')?.textContent,
		).toContain('That recovery link could not be used');
