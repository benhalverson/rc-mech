...
const componentFormFrom = (component: InstalledComponent): ComponentForm => ({
	slotType:
		component.slotType ??
		(standardComponentSlots.includes(component.slot as never)
			? 'standard'
			: 'custom'),
	slot: component.slot,
	name: component.name,
...
const emptyDriveSessionForm = (): DriveSessionForm => ({
	startedAt: '',
	durationMinutes: '',
	conditions: '',
	notes: '',
...
const driveSessionPayload = (
	form: DriveSessionForm,
	timezone: string,
): {
	startedAt: string;
	conditions: string;
...
export class GaragePages {
	...
	protected readonly componentForm = signal<ComponentForm>(
		emptyComponentForm(),
	);
	...
	protected readonly sessionForm = signal<DriveSessionForm>(
		emptyDriveSessionForm(),
	);
	...
	protected readonly componentGroups = computed(() =>
		this.componentSlots().map((slot) => {
			const history = this.components()
	...
	protected readonly hasComponentHistory = computed(() =>
		this.componentGroups().some(
			(group) => Boolean(group.current) || group.history.length > 0,
		),
	);
	...
	private loadCars(): void {
		...
		this.http.get<CarsResponse>('/api/v1/cars', options).subscribe({
			next: ({ cars }) => {
				this.cars.set(cars);
				this.activeCars.set(
					String(cars.filter((car) => !car.archivedAt).length),
				);
	...
	protected openAddSession(): void {
		...
		this.sessionForm.set({
			...emptyDriveSessionForm(),
			startedAt: localDateTimeForTimezone(
	...
	protected saveSession(): void {
		...
		const request =
			mode === 'edit' && sessionId
				? this.http.patch<DriveSessionResponse>(
						`/api/v1/cars/${car.id}/drives/${sessionId}`,
						driveSessionPayload(form, this.timezone()),
						{ withCredentials: true },
					)
				: this.http.post<DriveSessionResponse>(
						`/api/v1/cars/${car.id}/drives`,
						driveSessionPayload(form, this.timezone()),
						{ withCredentials: true },
					);
		request.subscribe({
	...
	protected openAddComponent(slot = ''): void {
		...
		this.componentForm.set({
			...
			slot: slot || 'motor',
			slotType:
				slot && !standardComponentSlots.includes(slot as never)
					? 'custom'
					: 'standard',
		});
	...
	protected openReplaceComponent(component: InstalledComponent): void {
		...
		this.componentForm.set({
			...emptyComponentForm(),
			slotType: component.slotType ?? 'custom',
			slot: component.slot,
		});
		this.componentFormError.set('');
		this.componentEditing.set(true);
	...
	protected updateComponentField(
	...
	protected saveComponent(): void {
		...
		if (!form.slot.trim() || !form.name.trim()) {
			this.componentFormError.set(
				'Choose a slot and describe the component before saving.',
			);
			return;
		}
		...
		const request =
			mode === 'edit' && componentId
				? this.http.patch<{ component: InstalledComponent }>(
						`/api/v1/cars/${car.id}/components/${componentId}`,
						componentDetailsPayload(form),
						{ withCredentials: true },
					)
				: mode === 'replace' && componentId
					? this.http.post<{ component: InstalledComponent }>(
							`/api/v1/cars/${car.id}/components/${componentId}/replace`,
							componentPayload(form),
							{ withCredentials: true },
						)
					: this.http.post<{ component: InstalledComponent }>(
			...
			next: () => {
				...
				this.message.set(
					mode === 'replace'
						...
						: mode === 'edit'
							? `${form.slot} details saved.`
							: `${form.slot} added to the build sheet.`,
				);
	...
	private loadDriveSessions(carId: string): void {
		...
		this.http
			.get<DriveSessionsResponse>(`/api/v1/cars/${carId}/drives`, {
				withCredentials: true,
				params: { history: 'true' },
			})
			.subscribe({
				next: (response) => {
					this.driveSessions.set(
						response.driveSessions ?? response.sessions ?? [],
					);
	...
	private loadTimezone(): void {
		...
		this.http
			...
			.subscribe({
				next: (response) => {
					const value =
						response.timezone && this.isValidTimezone(response.timezone)
							? response.timezone
							: defaultTimezone();
	...
	protected saveCar(): void {
		...
		const request = id
			? this.http.patch<CarResponse>(`/api/v1/cars/${id}`, carPayload(form), {
					withCredentials: true,
				})
			: this.http.post<CarResponse>('/api/v1/cars', carPayload(form), {
					withCredentials: true,
				});
		request.subscribe({
			next: ({ car }) => {
				...
				this.message.set(
					id ? 'Car details saved.' : 'Car added to the garage.',
				);
				this.loadCars();
			},
			error: (error: { status?: number }) => {
	...
	protected archiveCar(): void {
		this.changeCarArchiveState('archive');
	}

	protected restoreCar(): void {
		this.changeCarArchiveState('restore');
	}
			...
			.subscribe({
				next: ({ car: updated }) => {
					this.cars.update((cars) =>
						cars.map((item) => (item.id === updated.id ? updated : item)),
					);
					this.selectedCarId.set(updated.id);
					this.carAction.set(null);
					this.message.set(
						action === 'archive'
							? 'Car archived. Its history is still available.'
							: 'Car restored to the active garage.',
					);
