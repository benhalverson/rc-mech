...
export class CarPhotoGallery {
  ...
  protected move(photo: CarPhoto, direction: -1 | 1): void {
    ...
    if (index < 0 || nextIndex < 0 || nextIndex >= this.photos().length || this.action()) return;
  ...
  protected designatePrimary(photo: CarPhoto): void {
    ...
    this.error.set('');
    this.http.patch<PhotoResponse>(this.photoEndpoint(photo), { isPrimary: true }, { withCredentials: true }).subscribe({
      next: ({ photo: updated }) => {
        this.photos.update((photos) => photos.map((item) => item.id === updated.id ? updated : { ...item, isPrimary: false, primary: false }));
        this.action.set(null);
      },
      error: (error: { status?: number }) => this.fail(error, 'The primary photo could not be saved.'),
    });
  }
  ...
  protected retry(): void { this.load(this.carId()); }

  private load(carId: string): void {
    this.state.set('loading');
    this.error.set('');
    this.http.get<PhotosResponse>(`/api/v1/cars/${carId}/photos`, { withCredentials: true }).subscribe({
      next: ({ photos }) => {
  ...
  private replace(photo: CarPhoto, file: File): void {
    if (!this.validate(file) || this.archived() || this.action()) return;
    this.sendFile(`${this.photoEndpoint(photo)}/replace`, file, `replace:${photo.id}`);
  }
  ...
  private sendFile(url: string, file: File, action: string): void {
    ...
    this.error.set('');
    this.http.post<PhotoResponse>(url, body, { withCredentials: true }).subscribe({
      next: ({ photo }) => {
        this.photos.set(this.ordered(action === 'upload' ? [...this.photos(), photo] : this.photos().map((item) => item.id === photo.id ? photo : item)));
        this.action.set(null);
      },
      error: (error: { status?: number }) => this.fail(error, action === 'upload' ? 'The photo could not be uploaded.' : 'The photo could not be replaced.'),
    });
  }
    ...
    this.error.set(error.status === 401
      ...
      : error.status === 403 || error.status === 404
        ...
        : error.status === 409
          ...
          : error.status === 413 || error.status === 415 || error.status === 422
