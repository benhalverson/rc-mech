...
export class MaintenanceCockpit {
  ...
  private finishLoad(records: ServiceRecord[]): void {
    ...
    this.activity.update((items) => items.length ? items : records.filter((record) => !record.deletedAt).map((record) => ({ id: record.id, planId: record.planId ?? undefined, action: record.planId ? 'Scheduled service' : 'Ad hoc service', occurredAt: record.performedAt, note: record.description })));
    this.state.set('ready'); this.loaded.set(true);
  }
  ...
  protected openServiceCreate(): void {
    const firstCar = this.garage().find((car) => !car.archivedAt);
    this.serviceForm.set({ ...emptyServiceForm(), carId: firstCar?.id ?? '', performedAt: this.localDateTime(new Date()) });
    this.serviceEditingId.set(null); this.servicePlanId.set(null); this.serviceError.set(''); this.loadComponents(firstCar?.id ?? ''); this.serviceEditing.set(true);
  }
  ...
  protected cancelServiceEdit(): void { this.serviceEditing.set(false); this.serviceEditingId.set(null); this.servicePlanId.set(null); this.serviceError.set(''); }
  protected updateService(field: keyof ServiceForm, value: string): void { this.serviceForm.update((current) => ({ ...current, [field]: value })); if (field === 'carId') this.loadComponents(value); }
  protected setHistoryFilter(value: 'active' | 'deleted'): void { this.historyFilter.set(value); }

  protected saveService(): void {
    const form = this.serviceForm(); const cost = form.cost.trim() ? Number(form.cost) : null;
  ...
  protected save(): void {
    ...
    const payload = { carId: form.carId, componentId: form.componentId || undefined, name: form.name.trim(), intervalUnit: calendar === null ? 'none' : form.calendarUnit, intervalValue: calendar === null ? 1 : calendar, ...(calendar !== null && form.calendarUnit === 'days' ? { intervalDays: calendar } : {}), intervalSessions: runs === null ? undefined : runs, baselineAt: form.baselineAt ? this.toIso(form.baselineAt) : undefined, baselineSessionCount: Number(form.baselineRuns) || 0 };
  ...
  protected transition(plan: MaintenancePlan, action: 'pause' | 'resume' | 'archive'): void {
    if (this.isReadOnly(plan)) return;
    this.action.set(`${action}:${plan.id}`);
    const request = this.http.post<PlanResponse>(`/api/v1/maintenance-plans/${plan.id}/${action}`, {}, { withCredentials: true });
  ...
  protected recordCarName(record: ServiceRecord): string { return this.carName(record.carId); }
  protected recordComponentName(record: ServiceRecord): string { return record.componentId ? this.componentName(record.componentId) : 'Garage service'; }
  protected recordCost(record: ServiceRecord): string { return record.cost == null ? 'No cost logged' : `${record.currency ?? 'USD'} ${record.cost.toFixed(2)}`; }
  ...
  private loadComponents(carId: string): void { if (!carId) { this.components.set([]); return; } this.http.get<ComponentsResponse>(`/api/v1/cars/${carId}/components`, { withCredentials: true }).subscribe({ next: ({ components }) => this.components.set(components.filter((component) => !component.removedAt)), error: () => this.components.set([]) }); }
}
